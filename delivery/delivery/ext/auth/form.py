...
class StoreEditForm(FlaskForm):
    name_store = wtf.StringField("Nome da Loja", [wtf.validators.DataRequired()])
    category_id = QuerySelectField(query_factory=lambda: Category.query.all(), allow_blank=True, get_label='name', blank_text='Selecione uma Categoria')
    user_id = QuerySelectField(query_factory=lambda: User.query.all(), allow_blank=True, blank_text='Seleciona um usuário')
...
class ItemsEditForm(FlaskForm):
    ...
    image = FileField("Image")
    price = wtf.FloatField("Preço", [wtf.validators.DataRequired()])
    store_id = QuerySelectField('ID da loja', query_factory=lambda: Store.query.all(), allow_blank=True, blank_text='Selecione uma loja' )
